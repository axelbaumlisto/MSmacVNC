...
- `capture_liveness_rearm_multidisplay` (FIX-C): a production regression neither test above could see - both only ever ran a single-display layout. Runs a real two-or-more-display server (`displayNumber = -2`) and covers two cases the shipped `oldestFrameStamp()` got backwards, measured live on 2026-09-12 (nine re-arms in ~2 minutes on an idle second display while the active one served a real, working session). Test A feeds a synthetic frame to display index 0 every 20ms - faster than the shrunk silence window - while NEVER feeding display index 1, for several real 1Hz watchdog ticks, and asserts zero re-arms: an idle-but-present second display must not by itself force a rebuild of a desk whose other display is fine. Skips just this half (not the whole file) on a host with fewer than two displays. Test B then lets every display go silent (no synthetic feed at all, real ScreenCaptureKit not expected to hold a grant either) and asserts `GiveUp` is still reached with `rearmSuccesses >= 3` and `rearmFailures == 0` - the SUCCESS-branch complement of `capture_liveness_rearm_failure`'s forced-failure case, proving a working rebuild with nothing to show for it cannot loop forever either. Skips visibly (77) if `GiveUp` never arrives within its own generous budget.

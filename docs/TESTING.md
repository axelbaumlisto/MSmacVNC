...
- `capture_liveness`: whether a technically-running capture stream is still delivering frames - idle/not-running is always alive, the first-frame grace window, the silence threshold boundary, the re-arm cooldown, `GiveUp` after the re-arm budget is spent, a fresh frame resetting the count, and the shipped grace/silence/cooldown/max-re-arms constants.
